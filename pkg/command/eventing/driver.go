...
import (
	...
	"knative.dev/kperf/pkg/command/eventing/driver"
)

func NewEventingDriverCommand(p *pkg.PerfParams) *cobra.Command {
	cleanArgs := cleanArgs{}
	ksvcCleanCommand := &cobra.Command{
		Use:   "driver",
		Short: "Run eventing workload driver",
		Long: `Run eventing workload driver

For example:
# To run locally Knative Eventing workload driver
kperf eventing driver
`,
		RunE: func(cmd *cobra.Command, args []string) error {

			fmt.Println("Eventng driver starting")
			driver.DriveWorkload()
			return nil
		},
	}
